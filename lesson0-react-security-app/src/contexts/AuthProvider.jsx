...
export const AuthProvider = ({ children }) => {
  ...
  // null이므로 앱이 사용자에게 로그아웃된 화면을 보여주다가 useEffect 실행 후 user 정보가 발견되면 갑자기 로그인된 화면으로 바뀌는 상황이 벌어질 수도 있음
  ...
  }, []); // 의존성 빈배열이므로 마운트시에 한번 실행

  // 로그인 처리 - 사용자 정보 저장
  // 사용자 정보를 상태(state)에 설정하고 로컬 저장소(localStorage)에 저장하는 로그인(login) 과정을 담당
  const login = (userData) => {
    setUser(userData); // 모두가 공유하는 React Context에 인증 사용자 정보를 저장 ->자식  리렌더링
